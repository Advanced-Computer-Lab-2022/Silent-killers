...
const Instructorfilter = () => { 
  const [Title,setTitle] = useState(null) //= price not title
...
//const [results,setresults] = useState(null)
  const [authors,setAuthors] = useState([]);
    ...
    const getAuthors =  async () => {
      await axios.post(`http://localhost:8000/api/Instructor/insfilter/?instructor=${localStorage.getItem("user")}`, {price:Title,subject:Subject}).then(
        (res) => { 
