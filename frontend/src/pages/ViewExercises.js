...
console.log(localStorage.getItem("user"));
  const [authors,setAuthors] = useState([]);
    ...
    const getAuthors =  async () => {
      await axios.post(`http://localhost:8000/api/IndividualTrainee/viewallExercises`).then(
        (res) => { 
      ...
      <Table sx={{ minWidth: 650 }} size="small" aria-label="a dense table">
        ...
        <TableBody>
          {authors.map((author) => (
            ...
            onClick={() => window.location.href=`/OpenExercise/?id=${author._id}`}
              key={author._id}
              

              >
              <TableCell align="center">{author.ExerciseNumber}</TableCell>
              <TableCell align="center">{author.CourseTitle}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
